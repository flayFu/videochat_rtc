...
- (void)_SR_commonInit;
{
    ...
    _workQueue = dispatch_queue_create(NULL, DISPATCH_QUEUE_SERIAL);//创建一个串行队列
...
{                
    ...
    if (!isControlFrame) {//如果不是控制帧
        [self _readFrameNew];//初始化设置 数据长度 opcode等
    } else {
        dispatch_async(_workQueue, ^{
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    if (self.readyState != SR_OPEN) {//如果没有打开就返回
        return;
    }
    
    
    BOOL isControlFrame = (frame_header.opcode == SROpCodePing || frame_header.opcode == SROpCodePong || frame_header.opcode == SROpCodeConnectionClose);//ping pong close 是控制帧
    
    if (isControlFrame && !frame_header.fin) {//控制帧fin应为1表示结束
    ...
    if (!isControlFrame) {
        _currentFrameOpcode = frame_header.opcode;//获得opcode
        _currentFrameCount += 1;//当前帧数量增加
    }
    
    if (frame_header.payload_length == 0) {//接收到的数据长度为0
        if (isControlFrame) {//是控制帧
            [self _handleFrameWithData:curData opCode:frame_header.opcode];
        } else {
            if (frame_header.fin) {//就这一个结束
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(SRWebSocket *self, NSData *data) {
        ...
        if (headerBuffer[0] & SRRsvMask) {//1-3 bit should be zero
            [self _closeWithProtocolError:@"Server used RSV bits"];
            return;
        }
        // codemask 00001111 opcode are the last four bits
        ...
        //continue frame opcode is previous frame opcode
...
{
    [self assertOnWorkQueue];//判断是否在工作队列
    
    if (nil == data) {
        return;
    }
    
    NSAssert([data isKindOfClass:[NSData class]] || [data isKindOfClass:[NSString class]], @"NSString or NSData");//判断是否是 data str
    
    size_t payloadLength = [data isKindOfClass:[NSString class]] ? [(NSString *)data lengthOfBytesUsingEncoding:NSUTF8StringEncoding] : [data length];
        
    NSMutableData *frame = [[NSMutableData alloc] initWithLength:payloadLength + SRFrameHeaderOverhead];//开辟需要存放数据的空间
    if (!frame) {
    ...
    // set fin
    frame_buffer[0] = SRFinMask | opcode;// srfinmask 0x80
    ...
    const uint8_t *unmasked_payload = NULL;//得到负载的大小
...
//    0 - 125 126 127 搞定负载的大小
    if (payloadLength < 126) {//负载小于126字节
        frame_buffer[1] |= payloadLength;
    } else if (payloadLength <= UINT16_MAX) {//65535
        frame_buffer[1] |= 126;
        *((uint16_t *)(frame_buffer + frame_buffer_size)) = EndianU16_BtoN((uint16_t)payloadLength);
        
        frame_buffer_size += sizeof(uint16_t);// 2 bytes
    } else {
        frame_buffer[1] |= 127;
        *((uint64_t *)(frame_buffer + frame_buffer_size)) = EndianU64_BtoN((uint64_t)payloadLength);
        
        frame_buffer_size += sizeof(uint64_t);// 8 bytes
    }
    ...
    if (!useMask) {
        ...
        frame_buffer_size += sizeof(uint32_t);//mask key 4 bytes
        
        // TODO: could probably optimize this with SIMD
        for (size_t i = 0; i < payloadLength; i++) {
            frame_buffer[frame_buffer_size] = unmasked_payload[i] ^ mask_key[i % sizeof(uint32_t)];//异或 不同为1
            frame_buffer_size += 1;
        }
    }
